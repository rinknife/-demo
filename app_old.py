...
def agent_chat(user_message, chat_history, last_products=None):
    ...
    # ====================== 核心修改：有记忆就用记忆，不重新查库 ======================
    if last_products is not None and len(last_products) > 0:
        # 👇 用户在上一轮已经获得推荐 → 直接用这些商品，不重新搜索
        products_dict = {}
        for i, p in enumerate(last_products):
            products_dict[f"memory_{i}"] = p
...
"""
    ...
    "products": products_dict  # 把商品带回，给下一轮记忆
}
...
def respond(message, chat_history):
    if chat_history is None:
        chat_history = []
        # 👇 新增：把用户最后一次的商品数据存在 chat history 里（持久记忆）
    last_products = chat_history[-1]["products"] if len(chat_history) > 0 and "products" in chat_history[-1] else None
    # 调用 agent，把记住的商品传进去
    bot_reply = agent_chat(message, chat_history, last_products)

    # 返回 messages 格式，Gradio 6.x 必须
    chat_history.append({"role": "user", "content": message})
    # 👇 修改：把本轮推荐的商品也存进历史
    chat_history.append({"role": "assistant", "content": bot_reply["reply"], "products": bot_reply["products"]})
    return "", chat_history
